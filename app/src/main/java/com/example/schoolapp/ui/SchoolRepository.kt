...
class SchoolRepository(private val dao: SchoolDao) {
    ...
    suspend fun addTeacherClass(teacherClassEntity: TeacherClassEntity) = dao.insertTeacherClass(teacherClassEntity)
    ...
    suspend fun deleteTeacherClass(teacherClassEntity: TeacherClassEntity) = dao.deleteTeacherClass(teacherClassEntity)
    ...
    fun getTeacherWithClassesById(teacherId: Int): LiveData<TeacherWithClasses> = dao.getTeacherWithClassesById(teacherId)
    fun getClassWithTeachers(classId: Int): LiveData<ClassWithTeachers> = dao.getClassWithTeachersById(classId)
}